...
- (void)viewDidLoad {
    ...
    @weakify(self);
    [self.webView addObserverBlockForKeyPath:LMJKeyPath(self.webView, estimatedProgress) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
		@strongify(self);
...
- (UIView *)lmjNavigationBarLeftView:(LMJNavigationBar *)navigationBar
{
    ...
    self.closeBtn.x = leftView.width - self.closeBtn.width;
...
- (UIProgressView *)progressView
{
    if(_progressView == nil && [self.parentViewController isKindOfClass:[UINavigationController class]])
    {
        ...
        progressView.width = viewWidth();
        
        progressView.y = self.lmj_navgationBar.height;
