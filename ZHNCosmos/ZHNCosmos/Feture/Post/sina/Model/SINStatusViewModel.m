...
- (NSMutableAttributedString *)sin_textPost
{
    if(_sin_textPost == nil && !LMJIsEmpty(self.status.text))
    {
        ...
        //        @weakify(self);
        NSMutableAttributedString *postTextM = [[NSMutableAttributedString alloc] initWithAttributedString:[[HMEmoticonManager sharedManager] emoticonStringWithString:self.status.text font:[UIFont systemFontOfSize:15] textColor:UIColor.redColor]];
        
        
        postTextM.yy_lineSpacing = 4.0;
        postTextM.yy_font = [UIFont systemFontOfSize:15];
...
- (CGFloat)postTextHeight
{
    if(_postTextHeight == 0)
    {
        ...
        CGSize textSize = CGSizeMake(viewWidth() - 2 * margin, INFINITY);
...
- (NSString *)sin_source
{
    if(_sin_source == nil)
    {
        ...
        attrM.yy_font = [UIFont systemFontOfSize:11];
...
- (NSString *)sin_cmtCount
{
    if(_sin_cmtCount == nil)
    {
        ...
        if (SNCompare(_sin_cmtCount, @"0") == NSOrderedSame) {
...
- (NSString *)sin_dingCount
{
    if(_sin_dingCount == nil)
    {
        ...
        if (SNCompare(_sin_dingCount, @"0") == NSOrderedSame) {
...
- (NSString *)sin_repostCount
{
    if(_sin_repostCount == nil)
    {
        ...
        if (SNCompare(_sin_repostCount, @"0") == NSOrderedSame) {
...
- (SINStatusPicsViewModel)sin_statusPicsViewModel
{
    if (_sin_statusPicsViewModel.cols == 0 && self.status.pic_urls.count) {
        ...
        CGFloat maxWidth = viewWidth() - 2 * margin;
        ...
        if (self.status.pic_urls.count == 0) {
            ...
            itemWidth = (viewWidth() - 2 * margin - 2 * itemMargin) / 3.0;
